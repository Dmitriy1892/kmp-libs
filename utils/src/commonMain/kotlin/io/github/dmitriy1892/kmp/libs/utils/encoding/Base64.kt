...
object Base64 {
    ...
    fun decode(ignoreSpaces: Boolean, str: String, url: Boolean = false): ByteArray {
        return if (ignoreSpaces) decodeIgnoringSpaces(str, url) else decode(str, url)
    }
    ...
    fun decodeIgnoringSpaces(str: String, url: Boolean = false): ByteArray {
        val regex = Regex("[\\s\\n\\r\\t]")
        val preparedString = regex.replace(str, "")
        return decode(preparedString, url)
    }
    ...
    fun decode(src: ByteArray, dst: ByteArray, url: Boolean = false): Int {
        val decodeArray = if (url) DECODE_URL else DECODE
    ...
    fun encode(src: ByteArray, url: Boolean = false, doPadding: Boolean = false): String {
        val encodeTable = if (url) TABLE_URL else TABLE
    ...
    private fun String.toDecodeArray(): IntArray = IntArray(
        size = 256,
        init = { -1 }
    ).also { intArray ->
        indices.forEach { index ->
            val charCode = this[index].code
            intArray[charCode] = index
        }
    }
}